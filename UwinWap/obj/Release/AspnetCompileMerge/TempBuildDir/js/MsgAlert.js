...
Showbo.Msg = {
    ...
    createBtn: function (p, v, fn, linkrul) {
        ...
        btn.onclick = function () {
            window.location = linkrul;
        }
        return btn;
    },
    alert: function (msg,linkrul) {
        this.show({ buttons: { yes: '确认' }, msg: msg, linkrul: linkrul });
    },
    ...
    show: function (cfg) {
        ...
        if (cfg.wait) {
            ...
            this.dvBottom.style.height = '95px';
            this.dvBtns.innerHTML = '<div class="height"></div>';
            if (cfg.buttons.yes) {
                this.dvBtns.appendChild(this.createBtn('yes', cfg.buttons.yes, cfg.fn, cfg.linkrul));
                if (cfg.buttons.no) this.dvBtns.appendChild(document.createTextNode('　'));
            }
            if (cfg.buttons.no) this.dvBtns.appendChild(this.createBtn('no', cfg.buttons.no, cfg.fn, cfg.linkrul));
        }
